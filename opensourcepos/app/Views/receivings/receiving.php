    ...
    <div class="flex flex-col gap-3">
        ...
        <div class="register-card">
            ...
            <div class="flex items-center gap-2">
                ...
                <?php if ($show_stock_locations) { ?>
                    <div class="flex items-center gap-1.5">
                        <label
                            class="text-sm font-semibold text-slate-700"><?= lang(ucfirst($controller_name) . '.stock_source') ?>:</label>
                    ...
                    <?php if ($mode == 'requisition') { ?>
                        <div class="flex items-center gap-1.5">
                            <label
                                class="text-sm font-semibold text-slate-700"><?= lang(ucfirst($controller_name) . '.stock_destination') ?>:</label>
        ...
        <div class="register-card">
            ...
            <button id="new_item_button"
                class="btn btn-primary btn-sm modal-dlg inline-flex items-center gap-2 shrink-0"
                data-btn-submit="<?= lang('Common.submit') ?>" data-btn-new="<?= lang('Common.new') ?>"
                data-href="<?= "items/view" ?>" title="<?= lang('Sales.new_item') ?>">
                    ...
                    <?php } else {
                        foreach (array_reverse($cart, true) as $line => $item) {
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors align-middle">
                                <td class="px-4 py-3">
                                    <a href="<?= site_url("$controller_name/deleteItem/$line") ?>"
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-col">
                                        <span
                                            class="font-medium text-slate-900"><?= esc($item['name'] . ' ' . implode(' ', [$item['attribute_values'], $item['attribute_dtvalues']])) ?></span>
                                        <span class="text-xs text-slate-500">[<?= to_quantity_decimals($item['in_stock']) ?> in
                                ...
                                <td class="px-4 py-3 text-center">
                                    <button type="submit" class="text-emerald-500 hover:text-emerald-700 transition-colors p-1"
                            ...
                            <?php if ($item['allow_alt_description'] == 1 || $item['description'] != '') { ?>
                                <tr class="bg-slate-50/30">
                                    <td class="px-4 py-1 text-[10px] text-slate-400 font-bold uppercase">
                                        <?= lang('Sales.description_abbrv') ?></td>
    ...
    <div class="flex flex-col gap-3">
        <div class="register-card p-5">
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col">
                                <span
                                    class="text-xs text-slate-500 font-medium"><?= lang(ucfirst($controller_name) . '.supplier') ?></span>
                                <span class="text-sm font-bold text-slate-900"><?= esc($supplier) ?></span>
                            </div>
                        </div>
                        <a href="<?= site_url("$controller_name/removeSupplier") ?>"
                            class="text-slate-400 hover:text-red-500 transition-colors"
                            title="<?= lang('Common.remove') ?>">
            ...
            <?php } else { ?>
                ...
                <label
                    class="text-xs font-bold text-slate-500 uppercase tracking-wider"><?= lang(ucfirst($controller_name) . '.select_supplier') ?></label>
                <div class="relative">
                    <?= form_input([
                        'name' => 'supplier',
                        'id' => 'supplier',
                ...
                <button id="new_supplier_button"
                    class="btn btn-default btn-sm modal-dlg inline-flex items-center justify-center gap-2"
                    data-btn-submit="<?= lang('Common.submit') ?>" data-href="<?= "suppliers/view" ?>"
            ...
            <div class="flex flex-col gap-4">
                ...
                <?php if (count($cart) > 0) { ?>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label
                            class="text-xs font-bold text-slate-500 uppercase tracking-wider"><?= lang('Common.comments') ?></label>
                        <?= form_textarea([
                            'name' => 'comment',
                            'id' => 'comment',
                    ...
                    <?php if ($mode != 'requisition') { ?>
                        <div class="grid grid-cols-1 gap-3 p-3 bg-white rounded-lg border border-slate-200">
                            <div class="flex items-center justify-between px-1">
                                <label
                                    class="text-xs font-medium text-slate-600"><?= lang(ucfirst($controller_name) . '.print_after_sale') ?></label>
                                <?= form_checkbox([
                            ...
                            <?php if ($mode == "receive") { ?>
                                <div class="flex flex-col gap-1">
                                    <label
                                        class="text-[10px] font-bold text-slate-400 uppercase"><?= lang(ucfirst($controller_name) . '.reference') ?></label>
                                    <?= form_input([
                                        'name' => 'recv_reference',
                                        'id' => 'recv_reference',
                            ...
                            <div class="flex flex-col gap-1">
                                <label
                                    class="text-[10px] font-bold text-slate-400 uppercase"><?= lang('Sales.payment') ?></label>
                                <?= form_dropdown(
                                    'payment_type',
                                    $payment_options,
                                    [],
                            ...
                            <div class="flex flex-col gap-1">
                                <label
                                    class="text-[10px] font-bold text-slate-400 uppercase"><?= lang('Sales.amount_tendered') ?></label>
                                <?= form_input([
                    ...
                    <div class="flex gap-2">
                        <button type="button" id="cancel_receiving_button"
                            class="flex-1 h-10 inline-flex items-center justify-center gap-2 rounded-md bg-white border border-slate-200 text-red-600 text-sm font-semibold hover:bg-red-50 hover:border-red-200 transition-colors">
                            <i data-lucide="x" class="w-4 h-4"></i>
                            <?= lang('Common.cancel') ?>
                        </button>
                        <button type="button" id="finish_receiving_button"
                            class="flex-[2] h-10 inline-flex items-center justify-center gap-2 rounded-md bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 shadow-sm shadow-emerald-200 transition-all">
