(function (dialog_support, $) {
    ...
    return function (dlog_ref) {
      ...
      if (button_id == "submit" && !submitted && btn_id != "btnNew") {
        ...
        if (validator.valid()) {
          ...
          // Invalidate cache on submit to ensure fresh CSRF tokens
          modal_cache = {};
        }
      }
      return false;
  ...
  var init = function (selector) {
    ...
    // Pre-fetch 'New' modals
    $(selector).each(function () {
      var $link = $(this);
      if ($link.data("btn-new")) {
        var url = $link.attr("href") || $link.data("href");
        if (url && !modal_cache[url]) {
          $.get(url, function (data) {
            modal_cache[url] = data;
          });
        }
      }
    });
            ...
            $.extend(
              {
                ...
                message: function () {
                  ...
                  if (modal_cache[url]) {
                    node.html(modal_cache[url]);
                    if (window.lucide) {
                      setTimeout(function () {
                        window.lucide.createIcons({ root: node[0] });
                      }, 0);
                    }
                  } else {
                    // Add loading spinner
                    node.html(
                      '<div class="flex justify-center items-center p-10"><i data-lucide="loader-2" class="w-10 h-10 animate-spin text-slate-400"></i></div>',
                    );

                    // Initialize Lucide icons for the spinner (scoped to node)
                    if (window.lucide) {
                      setTimeout(function () {
                        window.lucide.createIcons({ root: node[0] });
                      }, 0);
